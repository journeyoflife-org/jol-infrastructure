...
class TestIngestEndpoint:
    ...
    def test_ingest_with_content_returns_201(
        ...
        with (
            patch("app.services.pipeline.get_pipeline") as mock_pipeline,
            patch("app.services.pipeline.get_embedding_service") as mock_embed,
            patch("app.services.pipeline.get_vectorstore_service") as mock_vs,
        ):
            mock_pipe = MagicMock()
    ...
    def test_ingest_without_content_or_path_returns_400(
        ...
        assert "content" in response.json()["detail"].lower() or "file_path" in response.json()["detail"].lower()
    ...
    def test_ingest_with_metadata(
        ...
        with patch("app.services.pipeline.get_pipeline") as mock_pipeline:
            mock_pipe = MagicMock()
