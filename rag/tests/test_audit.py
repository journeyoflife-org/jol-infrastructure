...
class TestAuditRecordStructure:
    ...
    def test_audit_record_has_required_fields(
        ...
        """Each audit record must have: timestamp, action, user_id, resource_id, client_ip, outcome."""
        with patch("app.services.pipeline.get_pipeline") as mock_pipeline:
            from app.models import QueryResponse
            from datetime import UTC, datetime
        ...
        required_fields = ["timestamp", "action", "user_id", "resource_id", "client_ip", "outcome", "service"]
