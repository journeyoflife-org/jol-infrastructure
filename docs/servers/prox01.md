...
| Service Tag   | JBJ1BW2                                    |
| CPU           | 2× Intel Xeon Gold 6138 @ 2.00 GHz (Skylake-SP, Model 85 Stepping 4), 20 cores per socket = 40 cores total; thread count pending `lscpu` (HT state) — recorded from `racadm hwinventory` 2026-08-30 |
| RAM           | 96 GB — 12× 8 GB DDR4-2400 single-rank, multi-bit ECC; 12 of 24 slots used (12 free) — recorded from `racadm hwinventory` 2026-08-30 |
...
| iDRAC         | iDRAC9 **Enterprise** (license verified via iDRAC session 2026-08-30) |
...
- **2026-08-23**: Phase 0 reachability probe from admin01 — iDRAC
  (10.10.10.11, VLAN 10) reachable; ALL VLAN 60/40 targets (10.60.60.1
  gateway, 10.60.60.10, 10.60.60.20, 10.40.40.10) return "No route to
  host": MikroTik RB5009 inter-VLAN path DOWN — consistent with the
  documented SPOF G6 / INC-20260814 pattern. Hardware inventory (D1)
  BLOCKED until edge restoration; racadm additionally requires an
  interactive iDRAC session (no secrets in automation). Evidence:
  `docs/compliance/evidence/execution/p0/prox01-reachability-probe-20260823-2026.txt`.
- **2026-08-30**: D1 hardware inventory attempt #2 — edge restored
  (vlan-audit-2026-08-29 F1): prox01 ICMP 0 % loss, PVE web UI (8006/TLS)
  serving, sshd completes KEX (OpenSSH_10.0p2 Debian-7+deb13u4, host key
  matches known_hosts) but **all key auth denied** (root + jol-admin, from
  workstation and via hv01) — fleet keys not yet authorized on this new
  host (provisioning gap, not a host fault). CPU/RAM/VM list remain
  ⚠ UNVERIFIED — manual check required via console/iDRAC virtual console;
  commands recorded in the evidence file. COMPLIANCE FLAG F-D1: sshd
  advertises `publickey,password` — contradicts the key-only SSH policy
  above; verify + remediate from the console. Evidence:
  `docs/compliance/evidence/execution/p0/prox01-hw-inventory-attempt-20260830-0223.txt`.
- **2026-08-30**: D1 hardware inventory **RECORDED** from iDRAC9 Enterprise
  session (`racadm hwinventory`, operator console): 2× Intel Xeon Gold 6138
  @ 2.00 GHz (20 cores/socket, 40 cores total; thread count pending
  `lscpu`), 96 GB DDR4-2400 ECC (12× 8 GB, 12/24 slots used). Flag F-D1
  (sshd password authentication) reported FIXED + VERIFIED in the same
  session; remote re-verification pending — fleet SSH keys still denied
  from the workstation (probe rc=255), authorized-key authorization owed.
  Capacity baseline re-computed with real numbers (0.50 RAM ratio — PASS);
  remaining D1 gates: `data` pool creation, ARC cap recording, `lscpu`.
