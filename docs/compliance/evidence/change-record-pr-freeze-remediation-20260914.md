...
|----|---------|----------|--------------|
| R1 | `jol-compliance` CI workflows broken (config, not code) — duplicate runs, persistent qodana/validation failures | MEDIUM | Separate CI-repair PR before #4 can go green |
| R2 | `feat/pages-step6` (#93) is 125 commits / ~770 files | MEDIUM | Split into reviewable slices before merge |
...
| R7 | **Dependabot re-open loop (Step 2 not durable)**: `jol-ecommerce-engine` stale PRs #15–22 closed 15:38–15:40Z → dependabot re-scan **re-opened 10 PRs #25–34 at 19:15Z** for the same deps (vite 8.2.0→8.3.0, stripe-js 9.12.1→9.16.0, eslint 10.8.0→10.10.0, react-stripe-js 6.8.0→6.10.0). `gh pr close --delete-branch` does NOT suppress a bump — only merge or `@dependabot ignore …` does. `jol-auth` additionally has **4 orphaned** `dependabot/*` branches (no open PR) from older superseded iterations. 12/13 repos held at 0 | MEDIUM | Comment `@dependabot ignore this major version` (or `ignore:` in dependabot.yml) for unwanted majors — do NOT blindly re-close, it recurs each check. Delete the 4 jol-auth orphan branches |
