...
| ~~R3~~ | **REMEDIATED 2026-09-16**: branch `JourneyOfLife-patch-2` already absent — not found on remote (`git ls-remote`), not local, no PRs reference it. Was deleted during PR freeze remediation cleanup (Step 6 marked it obsolete: main has `qodana-action@v2025.1` > branch `@v2024.1`) | **CLOSED** | `git ls-remote origin | grep patch-2` → empty; `gh pr list --search JourneyOfLife-patch-2` → [] |
| ~~R4~~ | **REMEDIATED 2026-09-16 (partial) + triaged**: CI debt survey across 3 repos identified 6 distinct failures. **FIXED**: jol-infrastructure Shell Script Lint — SC2015 in `git-fleet-sync.sh` L80 (cd+pipe pattern → `git -C`) and L178 (nested ternary → if/elif/else). **Documented as owner-action debt**: (1) jol-mcp-servers qodana — QODANA_TOKEN expired (same as R1, needs token rotation), (2) jol-hub Gitleaks — `GITLEAKS_LICENSE` empty (org requires paid license), (3) jol-hub SCA Backend — `safety check` exit 64 (real Python dep vulns), (4) jol-hub SCA Frontend — `pnpm audit` critical (Next.js middleware authz bypass CVE), (5) jol-hub WCAG/backend/frontend lint — pre-existing on main (confirmed by docs-only PR #95 triggering same failures). All 5 are pre-existing, not regressions | **CLOSED** | shellcheck + bash -n clean on `git-fleet-sync.sh`; 5 remaining items require owner actions (token rotation, license purchase) or code-level dep updates in jol-hub |
| ~~R5~~ | **REMEDIATED 2026-09-16**: fleet-wide dependabot.yml tuning across 8 repos. **Dead reviewers fixed**: jol-analytics-ai (`jol-platform-team`→`backend`, `jol-devops-team`→`devops`), jol-infrastructure (`platform-team`→`devops`). **Limits reduced** (10→5): jol-auth, jol-bitrix24-integration, jol-link-registry, jol-repo-template, jol-mcp-servers. **npm limit** (15→10): jol-hub PR #97. **Ignore rules added**: jol-mcp-servers (codeql-action>=4, qodana-action>=2027). **PRs closed**: mcp-servers #17, #19 (codeql-action 3→4 major bumps, suppressed by ignore). mcp-servers PR #20 + jol-hub PR #97 created for protected-branch repos | **CLOSED** | 6 direct API commits + 2 PRs (mcp #20, hub #97); 2 dependabot PRs closed; 0 dead reviewers remaining |
...
| ~~R8~~ | **REMEDIATED 2026-09-16**: two-layer enforcement model captured as a durable runbook at `docs/runbooks/github-branch-protection-audit.md` (165 lines). Covers: classic ∪ rulesets effective-posture formula, the `~DEFAULT_BRANCH` exclusion trap, per-repo audit script with `gh api` commands, CODEOWNERS dead-team verification, and the ruleset-by-ID read-back requirement (list endpoint omits `.rules`). Future audits must follow this runbook before asserting a gate | **CLOSED** | `docs/runbooks/github-branch-protection-audit.md` — 6-step procedure + audit script |
...
| 2026-09-16 | **R3 CLOSED — branch `JourneyOfLife-patch-2` already absent**: verified not on remote, not local, no PRs reference it. Was deleted during PR freeze remediation cleanup (Step 6 marked it obsolete) | `git ls-remote origin | grep patch-2` → empty; `gh pr list --search JourneyOfLife-patch-2` → [] |
| 2026-09-16 | **R4 CLOSED — CI debt triaged + 1/6 fixed**: CI survey across jol-infrastructure, jol-mcp-servers, jol-hub identified 6 failures. Fixed: Shell Script Lint SC2015 in `git-fleet-sync.sh` (2 patterns refactored). Documented as accepted debt: qodana token expired (mcp), Gitleaks license missing (hub), SCA backend/frontend vulns (hub), WCAG+lint pre-existing (hub). All 5 remaining are pre-existing, not regressions | `shellcheck git-fleet-sync.sh` → clean; `gh run view --log-failed` per repo |
| 2026-09-16 | **R5 CLOSED — fleet-wide dependabot tuning**: 8 repos tuned. Dead reviewers fixed: jol-analytics-ai (`jol-platform-team`→`backend`, `jol-devops-team`→`devops`), jol-infrastructure (`platform-team`→`devops`). Limits 10→5 on 5 repos, npm 15→10 on jol-hub. Ignore rules added on jol-mcp-servers (codeql-action>=4, qodana-action>=2027). mcp-servers #17/#19 closed (major bumps suppressed). PRs created: mcp #20, hub #97 (protected branches) | 6 direct API commits + 2 PRs; `gh pr view 17/19 --json state` → CLOSED |
| 2026-09-16 | **R8 CLOSED — two-layer enforcement runbook created**: `docs/runbooks/github-branch-protection-audit.md` captures the classic ∪ rulesets audit methodology, the `~DEFAULT_BRANCH` exclusion trap, ruleset-by-ID read-back requirement, effective-posture computation, and CODEOWNERS dead-team verification. All PR-freeze findings now CLOSED | `docs/runbooks/github-branch-protection-audit.md` (165 lines, 6-step procedure) |
