...
| Marketplace | `/opt/jolarca` | Payments, KYC/AML, VAT OSS | PCI-DSS, GDPR, EU VAT OSS Directive |

**Rule**: These trees NEVER merge into a single audit surface. Shared code between trees requires a DPIA.

> **Naming (2026-08-31)**: the marketplace tree and its repo fleet were renamed `jol-m-*` → `jolarca*` (`jolarca`, `jolarca-infrastructure`, `jolarca-compliance`, `jolarca-legal`, `jolarca-data`; tree `/opt/jol-m` → `/opt/jolarca`). The `jolarca*` fleet is NOT part of the church-tree ecosystem map above. Documents dated before 2026-08-31 retain legacy names; resolve them via `docs/compliance/rename-registry-jolm-to-jolarca.md`.
>
> **Org transfer (2026-09-02)**: all 5 `jolarca*` repos were transferred from `journeyoflife-org` to the separate `jolarca-dev` GitHub organization (ISO 27001 A.8.13 segregation — commercial/marketplace repos must not commingle with ministry repos in the same org). Frozen `jol-m-*` names remain in `journeyoflife-org` as redirect pointers (2-hop chain verified intact).
...
> runtime via `src/app/[...slug]`. `jol-frontend-platform` was retired on 2026-09-11 — it was
> an undeveloped 3-file scaffold superseded by that model; see
> `docs/compliance/evidence/change-record-retire-jol-frontend-platform-20260911.md`.
...
echo 'JOL_MCP_GIT_REPO_ROOT=/opt/jol/repos' >> /etc/jol-mcp/mcp.env   # keep 640 root:mcp-svc (world bits 0)
# 3. Provision an inspectable WORK TREE (bare repos under /opt/jol/git fail
#    'git status' — needs a work tree). Final ownership MUST be mcp-svc:mcp-svc
#    (git safe.directory rejects owner != invoking user) — run the clone as
#    jol-admin with a SCOPED safe.directory, then hand over to the service acct:
sudo -u jol-admin git -c safe.directory=/opt/jol/git/jol-mcp-servers.git \
  clone --depth 1 /opt/jol/git/jol-mcp-servers.git /opt/jol/repos/jol-mcp-servers
chown -R mcp-svc:mcp-svc /opt/jol/repos/jol-mcp-servers
...
**Nature (verified 2026-08-18, C1 resolved 2026-09-19)**: **Definition-only repository — NO RUNTIME exists or is planned in this repo.** `main.py` is a bootstrap/validator only ("Runtime orchestration lives elsewhere"). Finding **C1 CRITICAL** (2026-08-13 audit) is **RESOLVED**: `jol-hermes-agents` is formally declared a declarative definition repo; the readiness gate for runtime deployment moves to whichever future repo implements the runtime orchestration layer. No runtime repo exists in the fleet today. Every runtime claim below is a *contract for a future runtime*, not a verified behaviour.
...
#### Audit Checklist — Verify Before Declaring Operational
> Repo-level gates (executable from any checkout); runtime gates activate only once a runtime repo is created and audited.
```text
...
□ Runtime repo:        docs/audit/AUDIT_REPORT.md C1 status               # ✅ RESOLVED 2026-09-19 — jol-hermes-agents is definition-only; readiness gate moves to future runtime repo
...
- **Downstream**: a future runtime repo (not yet created) will consume this repo's config/skills/memory contracts; C1 resolved 2026-09-19 — readiness gate moves to that repo
...
5. **Hermes runtime (C1 CRITICAL, audit 2026-08-13)**: ✅ RESOLVED 2026-09-19 — `jol-hermes-agents` formally declared definition-only; readiness gate moves to future runtime repo. No runtime repo exists in the fleet today. MCP/Ollama integrations remain aspirational pending ADRs
